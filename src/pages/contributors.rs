...
#[server]
pub async fn get_contributors() -> Result<ContributorsResponse, ServerFnError> {
    Ok(fetch_contributors().await)
}
...
async fn fetch_contributors() -> ContributorsResponse {
    ...
    for c in &all_contributors {
        println!("  {} => contributions={}", c.login, c.contributions);
    }
    println!("=== END ({} contributors) ===\n", all_contributors.len());
...
async fn fetch_all_contributors() -> Vec<Contributor> {
    ...
    for repo in &repos {
        ...
        println!("  Repo '{}': {} contributors (since 2023)", repo.name, repo_count);
        ...
        match res {
            Ok(r) => {
                ...
                if status == reqwest::StatusCode::ACCEPTED {
                    println!(
                        "    Stats for '{}' still computing (attempt {}), waiting...",
                        repo,
                        attempt + 1
                    );
...
pub fn Contributors() -> impl IntoView {
    let contributors = Resource::new(|| (), |_| get_contributors());
            ...
            <section class="bg-orange-300/30 dark:bg-transparent py-16 min-h-[80vh]">
                <div class="flex flex-col gap-y-6 container mx-auto px-4">
                    ...
                    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5 gap-6">
                        <Suspense fallback=|| ()>
                            {move || {
                                contributors.get().and_then(|result| result.ok()).map(|data| {
                                    data.contributors.iter().map(|item| {
                                        view! {
                                            <ContributorCard
